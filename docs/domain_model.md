...
: メンバー / チーフ / マスター
- カレンダー閲覧許可
: メンバーが同じグループのスタッフにカレンダー閲覧を許可する関係
- システム設定
...
- 不変条件
  - staffId は自動割り付け、一意
  - メンバーは email 必須
  - role が メンバー または チーフ の場合、groupId は必須
...
- 不変条件
  ...
  - requester は MEMBER に限定する
  - target は requester と同じグループに所属する有効なスタッフ（MEMBER / CHIEF / MASTER）
...
- CalendarViewPermissionService
  ...
  - ルール
    ...
    - 申請対象一覧は同じグループの有効なスタッフを返し、申請者自身は除外する
...
- メンバーはスタッフ個別カレンダー画面のみアクセス可能
- チーフは管理者画面アクセス不可
- チーフは同一グループのメンバーに対してのみシフト確認・編集可能
- マスターは管理画面を含む全機能にアクセス可能
- メンバーから同じグループのスタッフへの閲覧申請は、管理者設定で機能有効時のみ利用可能
